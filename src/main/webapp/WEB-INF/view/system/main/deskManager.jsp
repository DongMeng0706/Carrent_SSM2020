    ...
    layui.use(['form','element','layer','jquery'],function(){
        ...
        $.get("${ctx}/news/loadAllNews.action?page=1&limit=10",function(data){
            ...
            for(var i=0;i<5;i++){
                ...
                if(data.data[i]!=null && typeof(data.data[i])!="undefined") {
                    var date = new Date();
                    var d = data.data[i].createtime;
                    date.setTime(data.data[i].createtime);
                    var newTime = date.Format("yyyy-MM-dd hh:mm:ss");

                    hotNewsHtml += '<tr onclick=viewNews(' + data.data[i].id + ')>'
                        + '<td align="left"><a href="javascript:;"> ' + data.data[i].title + '</a></td>'
                        + '<td>' + newTime.substring(0, 10) + '</td>'
                        + '</tr>';
                }
            }
